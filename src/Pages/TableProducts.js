    ...
    axios
      ...
      .then(({ data }) => {
        if (type === "0") {
          ...
          filterData = productsFilterType(data, "أدوات زجاجية");
          setProducts(filterData);
        } else if (type === "2") {
          filterData = productsFilterType(data, "أطقم سفرة");
          setProducts(filterData);
        } else if (type === "3") {
          filterData = productsFilterType(data, "شاي وجاتوه");
          setProducts(filterData);
        } else if (type === "4") {
          filterData = productsFilterType(data, "أركوبيركس");
          setProducts(filterData);
        } else if (type === "5") {
          filterData = productsFilterType(data, "شوك ومعالق");
          setProducts(filterData);
        } else if (type === "6") {
          filterData = productsFilterType(data, "تقديم");
          setProducts(filterData);
        } else if (type === "7") {
          filterData = productsFilterType(data, "خذف وبورسلين");
          setProducts(filterData);
        } else if (type === "8") {
          filterData = productsFilterType(data, "وبونشانيا");
          setProducts(filterData);
        } else {
          setProducts(data);
        }
